...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n):
        lower_left_corner_x = (rect.get_center().x - (k + 1) * delta) - (rect.get_width() / 2)
        lower_left_corner_y = (rect.get_center().y + (k + 1) * delta) + (rect.get_height() / 2)
        upper_right_corner_x = (rect.get_center().x + (k + 1) * delta) + (rect.get_width() / 2)
        upper_right_corner_y = (rect.get_center().y - (k + 1) * delta) - (rect.get_height() / 2)
        new_rect = rg.Rectangle(rg.Point(lower_left_corner_x, lower_left_corner_y), rg.Point(upper_right_corner_x, upper_right_corner_y))
